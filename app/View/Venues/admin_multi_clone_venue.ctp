...
<?php echo $this->Form->create('Venue');?>
	...
	<?php
                ...
                echo "<fieldset><legend>Copy {$i}</legend>";
                echo $this->Form->input($i . '.address');
                echo $this->Form->input($i . '.phone_1', array('class' => 'phone_field'));
                echo '<div id="hours_block">
                        <legend>Hours</legend>';
		echo $this->Form->input($i . '.hours_sun', array('after' => '<a href="#" name="Venue' . $i .'" class="link_copy_hours">Copy Sun.</a>'));
		echo $this->Form->input($i . '.hours_mon');
		echo $this->Form->input($i . '.hours_tue');
		echo $this->Form->input($i . '.hours_wed');
		echo $this->Form->input($i . '.hours_thu');
		echo $this->Form->input($i . '.hours_fri');
		echo $this->Form->input($i . '.hours_sat');
                echo $this->Form->input($i . '.ignore_hours', array('type' => 'checkbox'));
                echo $this->Form->input($i . '.website');
                echo '</div>';
                echo '</fieldset>';
